#!/usr/bin/env python3
"""Convert the article markdown to a PDF with the hero image embedded."""
...
ARTICLE_MD = os.path.join(WORKSPACE, "pipeline/drafts/2026-05-10-self-hosted-ai-agent-stack-replaces-saas.md")
HERO_IMG = os.path.join(WORKSPACE, "pipeline/images/2026-05-10-self-hosted-ai-agent-stack-hero.png")
OUTPUT_PDF = os.path.expanduser("~/Documents/AgentForge/2026-05-10-self-hosted-ai-agent-stack-replaces-saas.pdf")
...
def build_pdf():
    ...
    styles.add(ParagraphStyle(
        "BrandCode", parent=styles["Code"],
        fontSize=9, leading=13, textColor=BRAND_DARK,
        backColor=BRAND_LIGHT, spaceAfter=8, fontName="Courier",
    ...
    while i < len(lines):
        ...
        if line.startswith("# "):
            flush_table()
            text = line[2:].strip()
            story.append(Paragraph(text, styles["BrandTitle"]))
            story.append(HRFlowable(width="100%", thickness=2, color=BRAND_BLUE, spaceAfter=8 * mm))
        elif line.startswith("## "):
            flush_table()
            text = line[3:].strip()
            story.append(Paragraph(text, styles["BrandH1"]))
        elif line.startswith("### "):
            flush_table()
            text = line[4:].strip()
            story.append(Paragraph(text, styles["BrandH2"]))
        elif line.strip() == "":
    ...
    story.append(Paragraph(
        "AutoRanker &mdash; Self-Hosted AI Agent Stack &mdash; 2026-05-10",
        styles["Footer"]
    ))
