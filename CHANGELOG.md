...
- **Cron Scheduler** — Native cron scheduler in `internal/cron/cron.go`. Supports standard cron-format (5-field) and `@every` shorthand (e.g., `@every 1h`, `@every 30s`). Pipeline cron triggers are auto-registered from config on startup. A `cron_schedule` tool is registered in the tool registry under the `automation` category. The scheduler fires due jobs via a goroutine-ticker loop, emitting execution events on the bus for downstream consumers.
